...
if (empty($_COOKIE['lang'])) {
    $defaultLang = 'ru_RU';
    $cookieSecure = !empty($_SERVER['HTTPS']) && strtolower($_SERVER['HTTPS']) !== 'off';
    setcookie('lang', $defaultLang, time() + 60 * 60 * 24 * 365, '/', '', $cookieSecure, true);
    $_COOKIE['lang'] = $defaultLang;
}

$citadelAmpDefaults = array(
...
foreach ($citadelAmpDefaults as $key => $value) {
...
if ($quietmode) {
	...
	$langKey = !empty($_COOKIE['lang']) ? $_COOKIE['lang'] : 'ru_RU';
